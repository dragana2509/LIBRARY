...
<?php
	...
	$u = $user->fetch(PDO::FETCH_OBJ);
		...
		if (isset($_POST['submit'])) {
			...
			if (!empty($username_form ) AND !empty($name_form) AND !empty($surname_form)and !empty($email_form)) {
				$query = "UPDATE users 
				...
				$result->execute(array(":username"=>$username_form, ":name"=>$name_form,":surname"=>$surname_form,":email"=>$email_form));
		?>
